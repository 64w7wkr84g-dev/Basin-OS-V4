...
If now is not the time, I can leave you alone. If you want to be kept on the list for future fund windows or tax-planning updates, I can do that instead. What is better?`
  ...
  function renderDashboard() {
    ...
    $('#page-dashboard').innerHTML = `
      <div class="panel lux" style="margin-bottom:16px"><div class="panel-body"><div class="grid grid-5">
...
Actions → Basin Radar Daily → Run workflow</pre>
  ...
  function renderPage() {
    ...
    const subtitles = {
      dashboard:'Command view for lead quality, ready work, research queues, and next actions.',
      radar:'Automated public-source discovery, Brave enrichment, NPI cross-reference, and contact-route scoring.',
      workflow:'Day 1 through Day 10 cadence control with notes, dispositions, follow-ups, and handoffs.',
      linkedin:'Manual verification queue for public LinkedIn profile URLs discovered by Brave.',
      pipeline:'Ready investor leads only. Research-only records stay out until promoted.',
      cpa:'Referral and CPA/tax professional pipeline.',
      sequence:'Compliant channel scripts by day and available contact route.',
      callcoach:'Call positioning, rebuttals, and compliance guardrails.',
      notes:'Searchable notes, also attached to each full lead card.',
      handoffs:'Printable director handoff sheets from active lead cards.',
      calendar:'Follow-ups created from workflow actions and lead-card notes.',
      analytics:'Source mix, contact route visibility, and radar quality indicators.',
      api:'Groq browser setup and Brave/GitHub Actions status.',
      settings:'Backup, restore, and local browser CRM controls.'
    };
    $('#pageTitle').textContent = titles[state.page] || 'Dashboard';
    const sub = $('#pageSubtitle');
    if (sub) sub.textContent = subtitles[state.page] || '';
    ...
    const examples = [
      {
        ...
        summary: 'Use GitHub Actions for automated sourcing and Brave enrichment.',
        score: 58,
        contactMethods: [],
        evidenceTrail: [],
