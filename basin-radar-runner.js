...
/**
 * Basin OS V6.7 Lead Factory Automation Runner
 *
 * Safe design:
...
function queueFor(contacts){
  ...
  if(contacts.some(c=>/phone/i.test(c.type)&&digits(c.value).length>=10)) return {queue:'Ready to Work',first:'Day 1: call or verify by phone; ask for correct email/direct contact if needed.'};
...
(async function main(){
  ...
  fs.writeFileSync(out('data/radar-run-log.json'),JSON.stringify({lastRunAt:generatedAt,status:'complete',...radar.stats,message:`V6.7 created ${associateReady.length} associate-ready leads.`},null,2));
  console.log(`V6.7 complete: ${associateReady.length} associate-ready, ${research.length} research, ${searchCount} public searches.`);
