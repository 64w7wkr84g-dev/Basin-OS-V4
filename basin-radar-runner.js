#!/usr/bin/env node
/**
 * Basin OS V4 Radar Runner
 * GitHub Actions backend:
 * - Brave Search public discovery
 * - Groq/Llama extraction and scoring
 * - Writes JSON files consumed by Next.js frontend
 *
 * No LinkedIn scraping. No automated outreach. No tax advice.
 */
...
function slug(value) {
  return clean(value).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 100) || `lead-${Date.now()}`;
}
...
function addEvidence(lead, source, url, whatItProves) {
  ...
  if (!lead.evidenceTrail.some(e => e.url === u)) {
    lead.evidenceTrail.push({ source: clean(source || "Public Source"), url: u, whatItProves: clean(whatItProves || "Public source evidence.") });
  }
}
...
function leadKey(lead) {
  return [lead.name, lead.company, lead.title, lead.sourceUrl, (lead.contactMethods || []).map(c => `${c.type}:${c.value}`).sort().join("|")].join("|").toLowerCase().replace(/\W+/g, "");
}
...
async function main() {
  console.log("Basin Radar V4 starting");
  ...
  for (const lead of all) {
    try {
      if (!hasContact(lead, "email") || !hasContact(lead, "linkedin")) await enrichLeadWithBrave(lead);
    } catch (error) {
  ...
  const output = {
    generatedAt: now(),
    engine: "Basin OS V4 Radar Runner",
    compliance: {
...
main().catch(async error => {
  ...
  const fallback = {
    generatedAt: now(),
    engine: "Basin OS V4 fallback output",
