#!/usr/bin/env node
/**
 * Basin OS V3.3 — Groq Terminal Radar Runner
 *
 * Serverless backend for GitHub Actions.
 * - Uses Brave Search API for public discovery.
 * - Uses Groq/Llama for intelligent lead parsing, person validation, CPA detection, scoring, and fit reason.
 * - Preserves JSON file output used by the GitHub Pages frontend.
 * - Preserves routing buckets: ready, linkedinVerify, cpaVerify, skipped.
 *
 * Compliance:
 * - No LinkedIn page scraping.
 * - Stores only public search-result URLs/snippets.
 * - No auto-send, auto-DM, auto-call, or tax advice.
 * - Accredited status is never assumed.
 */
...
if (typeof fetch !== "function") {
  console.error("Fatal: native fetch() is unavailable. Use Node 20+.");
  process.exit(1);
}
...
function addEvidence(lead, source, url, whatItProves) {
  ...
  if (!u) return;
  lead.evidenceTrail ||= [];
  if (!lead.evidenceTrail.some(e => e.url === u)) {
    lead.evidenceTrail.push({
      source: clean(source || "Public Source"),
      url: u,
      whatItProves: clean(whatItProves || "Public source evidence.")
    });
  }
}
...
function defaultExtraction(textSnippet) {
  ...
  return {
    ...
    title: null,
    isPerson: false,
    isCPA: /cpa|tax|account/i.test(snippet),
    score,
    grade: gradeFromScore(score),
    fitReason: "Default fallback score because AI parsing was unavailable. Manual verification required."
  };
}
...
/**
 * Groq extraction function required by the upgrade.
 * Returns ONLY normalized schema data and never throws past caller unless network/runtime fails before fallback.
 */
...
- fitReason must be compliance-safe and explain why this is worth manual review.`;
  ...
  const userPrompt = clean(textSnippet).slice(0, 7000);
 ...
 * - Uses Groq for parsing/scoring/person/CPA detection.
 * - Contact-route extraction remains deterministic for email/phone/LinkedIn URL.
 * - If Groq fails, fallback score is 50.
 */
async function processSearchResultIntoLead(result, sourceContext = {}) {
  ...
  let extracted;
  try {
    if (runtime.groqCalls >= MAX_GROQ_CALLS) {
      extracted = defaultExtraction(resultText);
    } else {
    ...
    extracted = {
      ...defaultExtraction(resultText),
      score: 50,
      grade: "D",
      fitReason: `AI parsing failed; manual review required. ${clean(error.message).slice(0, 120)}`
    };
  }
  ...
  const sourceType = extracted.isCPA
    ? "cpa"
    : linkedinUrl
      ? "linkedin"
      : sourceContext.sourceType || "public";
  ...
  addEvidence(
    lead,
    sourceContext.source || "Brave Public Search",
    linkedinUrl || result.url,
    result.description || result.title || "Public search result evidence."
  );
...
function dedupeLeads(leads) {
  ...
  for (const lead of leads || []) {
    ...
    map.set(key, {
      ...
      evidenceTrail: uniqueBy([...(existing.evidenceTrail || []), ...(lead.evidenceTrail || [])], e => `${e.source}|${e.url}`.toLowerCase())
    });
  }
...
/**
 * Routing uses Groq-determined isPerson, isCPA, and score.
 */
function routeLead(lead) {
  ...
  lead.sourceConfidence = [
    ...
    `${(lead.evidenceTrail || []).length} EVIDENCE`
  ].filter(Boolean).join(" + ");
...
async function fetchBraveQueryLeads(query, count, sourceContext) {
  ...
  for (const result of response.results || []) {
    const lead = await processSearchResultIntoLead(result, {
      ...sourceContext,
      query
    });
...
async function fetchLinkedInDiscovery() {
  ...
  for (const query of queries) {
    ...
    const leads = await fetchBraveQueryLeads(query, 10, {
  ...
  for (const feed of feeds) {
    ...
    try {
      ...
      for (const item of xml.split("<item>").slice(1, 18)) {
        ...
        const lead = await processSearchResultIntoLead(
          { title, url: link, description },
          {
            source: "Google News RSS",
            sourceType: /cpa|tax|account/i.test(`${title} ${description}`) ? "cpa" : "rss",
  ...
  for (const taxonomy of taxonomies) {
    for (const state of states) {
      ...
      try {
        ...
        for (const row of json.results || []) {
          ...
          try {
            if (runtime.groqCalls < MAX_GROQ_CALLS) {
              ...
              extracted = defaultExtraction(snippet);
              extracted.name = name;
              extracted.title = taxonomy;
              extracted.isPerson = true;
            }
          } catch {
            runtime.groqFailures += 1;
            extracted = {
              ...
              score: 50,
              grade: "D",
              fitReason: "NPI provider record. Manual review required."
            };
          }
          ...
          const lead = {
            ...
            title: extracted.title || taxonomy,
            location,
            sourceType: "npi",
            source: "NPI Registry",
            sourceUrl,
            signal: `NPI provider record: ${name}`,
            summary: snippet,
            fitReason: extracted.fitReason,
            accreditedLikelyReason: "Professional role/context supports screening only. Accredited status must be confirmed by the prospect.",
            contactMethods: [],
            ...
            grade: extracted.grade || gradeFromScore(extracted.score || 50),
            type: "investor",
            isPerson: Boolean(extracted.isPerson),
            isCPA: Boolean(extracted.isCPA),
            foundAt: now()
          };
        ...
        console.warn("NPI fetch failed:", error.message);
      }
...
async function enrichLeadWithBrave(lead) {
  ...
  for (const result of response.results || []) {
    const email = extractEmail(`${result.title} ${result.url} ${result.description}`);
    const phone = extractPhone(`${result.title} ${result.url} ${result.description}`);
...
async function writeFallbackOutput(error) {
  const output = {
    generatedAt: now(),
    engine: "Basin OS V3.3 fallback output",
  ...
  await writeJson("data/radar-rejected.json", { generatedAt: now(), errors: output.errors });
  await writeJson("radar-rejected.json", { generatedAt: now(), errors: output.errors });
}

async function main() {
  console.log("Basin Radar V3.3 starting");
  ...
  try { linkedin = await fetchLinkedInDiscovery(); }
  catch (error) { errors.push({ source: "linkedinDiscovery", reason: error.message }); }

  try { cpa = await fetchCpaDiscovery(); }
  catch (error) { errors.push({ source: "cpaDiscovery", reason: error.message }); }

  try { rss = await fetchRssSeeds(); }
  catch (error) { errors.push({ source: "rss", reason: error.message }); }

  try { npi = await fetchNpiSeeds(); }
  catch (error) { errors.push({ source: "npi", reason: error.message }); }

  let all = dedupeLeads([...linkedin, ...cpa, ...rss, ...npi])
    .filter(lead => !radarState.suppressed?.[leadKey(lead)]);
  ...
  for (const lead of all) {
    try {
      ...
      errors.push({ source: "enrich", name: lead.name, reason: error.message });
    }
    enriched.push(lead);
  }
  ...
  all = dedupeLeads(enriched)
    ...
    .sort((a, b) => (a.priorityRank || 9) - (b.priorityRank || 9) || Number(b.score || 0) - Number(a.score || 0) || a.name.localeCompare(b.name));
  ...
  const output = {
    generatedAt: now(),
    engine: "Basin OS V3.3 Groq Terminal Radar Runner",
    compliance: {
    ...
    routingRules: {
      ready: "Groq says real person + email + score >= 58.",
      linkedinVerify: "Groq says real person + LinkedIn URL but no email.",
      cpaVerify: "Groq says real person + CPA/tax/referral relevance.",
...
main().catch(async error => {
  try {
    ...
    console.error("Fallback writer failed:", fallbackError);
